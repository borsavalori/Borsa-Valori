...
function onLoginSuccess(user) {
  ...
  if (DEMO_MODE) {
    ...
    navigateTo(isMobiel ? 'planning' : 'home');
    return;
  }
...
function renderSidebar() {
  ...
  if (kandidatenZichtbaar && (isAdmin() || demoStore.introActief)) {
    html += `
  ...
  el.innerHTML = `
    ...
    <div style="display:grid;grid-template-columns:1fr 3fr;gap:20px;align-items:start;">
      ...
      <div style="display:flex;flex-direction:column;gap:12px;">
        ...
        ${(isAdmin() || demoStore.kandidatenZichtbaar) && (isAdmin() || demoStore.introActief) ? `
...
function laadHomeActiviteit() {
  const actEl = document.getElementById('home-activiteit');

  if (DEMO_MODE) {
    if (actEl) actEl.innerHTML = '<div class="empty-state" style="padding:16px 0;"><p>Geen activiteit in demo-modus.</p></div>';
    return;
  }

  // Recente beleggingsboeken
  db.collection('beleggingsboeken').get().then(snap => {
    const items = [];
    snap.forEach(doc => items.push({ ...doc.data(), id: doc.id }));
    items.sort((a, b) => (b.aangemaakt || 0) - (a.aangemaakt || 0));
    const recent = items.slice(0, 3);

    const el = document.getElementById('home-activiteit');
    if (!el) return;
    if (recent.length === 0) {
      el.innerHTML = '<div class="empty-state" style="padding:16px 0;"><p>Nog geen activiteit.</p></div>';
      return;
    }
    el.innerHTML = recent.map(b => `
      <div style="display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid var(--grijs);">
        <span style="font-size:20px;">📚</span>
        <div>
          <div style="font-size:13px;font-weight:600;">${escapeHtml(b.name)}</div>
          <div style="font-size:11px;color:var(--grijs-donker);">Boek toegevoegd${b.auteur ? ' · ' + escapeHtml(b.auteur) : ''} · ${escapeHtml(b.date || '')}</div>
        </div>
      </div>
    `).join('');
  }).catch(() => {});
...
function planningRowHTML(row, canEditCells) {
  ...
  const pitchenOudVeld = canEditCells
  ...
  const pitchenNieuwVeld = canEditCells
  ...
  const icsKnop = (!adminOnly && row.datum)
    ? `<button class="btn btn-outline btn-sm" onclick="downloadIcs('${row.datum}','${(row.typeAvond||'').replace(/'/g,"\\'")}','${(row.hosten||'').replace(/'/g,"\\'")}')">📅</button>`
    : '';
...
function downloadIcs(datum, typeAvond, hosten) {
  ...
  const ics = [
    'BEGIN:VCALENDAR',
    'VERSION:2.0',
    'PRODID:-//Borsa Valori//NL',
    'BEGIN:VEVENT',
    `DTSTART;VALUE=DATE:${datStr}`,
    `DTEND;VALUE=DATE:${datStr}`,
    `SUMMARY:${typeAvond || 'Borsa Valori Avond'}`,
    `DESCRIPTION:Host: ${hosten || '—'} | Borsa Valori B&R Beurs`,
    'END:VEVENT',
    'END:VCALENDAR'
  ].join('\r\n');
  ...
  a.download = (typeAvond || 'borsa-avond').replace(/[^a-z0-9]/gi,'_') + '_' + datStr + '.ics';
  a.click();
  showToast('Toegevoegd aan agenda! 📅');
}

function savePlanningCell(id, field, value) {
  const adminOnlyFields = ['datum', 'typeAvond'];
...
function addPlanningRow() {
  ...
  const row = { id, datum: '', typeAvond: '', locatie: '', hosten: '', helpen: '', pitchenOud: '', pitchenNieuw: '' };
...
// ─── PAGE: LEDEN ─────────────────────────────────────────────────────────────
// Iedereen ziet dit tabblad. Admin mag elke rij volledig bewerken en nieuwe
// leden toevoegen. Een gewoon lid/Reisco-lid mag alleen de eigen rij bewerken
// (gekoppeld via 'koppelUsername' aan de ingelogde gebruikersnaam).

function magLedenRijBewerken(lid) {
  if (isAdmin()) return true;
  const mijnUsername = currentUserData.username || '';
  return lid.koppelUsername && lid.koppelUsername.toLowerCase() === mijnUsername.toLowerCase();
}
...
function renderLeden(el) {
  el.innerHTML = `
    ...
    <div class="alert alert-info" style="margin-top:16px;">
      <strong>ℹ️ Rechten:</strong> Je kunt alleen je eigen gegevens bewerken (grijze velden zijn van anderen en alleen door een Admin aan te passen).
      ${isAdmin() ? ' Als Admin kun je alle velden van iedereen bewerken.' : ''}
    </div>
  `;

  if (DEMO_MODE) {
    const tbody = document.getElementById('leden-tbody');
    if (tbody) tbody.innerHTML = demoStore.leden.map(lid => ledenRowHTML(lid)).join('');
    return;
  }
  ...
  db.collection('leden').get().then(snap => {
    ...
    if (tbody) {
      tbody.innerHTML = leden.length === 0
        ? `<tr><td colspan="8" style="text-align:center;padding:20px;color:var(--grijs-donker);">Nog geen leden. Voeg ze toe via "+ Lid toevoegen".</td></tr>`
        : leden.map(lid => ledenRowHTML(lid)).join('');
    }
  }).catch(e => {
    const tbody = document.getElementById('leden-tbody');
