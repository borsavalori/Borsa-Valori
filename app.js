...
function mijnLidRij() {
  const mijnUsername = (currentUserData && currentUserData.username || '').toLowerCase();
  if (!mijnUsername) return null;
  return (demoStore.leden || []).find(l =>
    l.koppelUsername && l.koppelUsername.toLowerCase() === mijnUsername
  ) || null;
}
