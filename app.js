...
// Eén normalisatie voor gebruikersnamen, gebruikt bij ZOWEL account aanmaken ALS inloggen.
// Voorheen strippte "+ Gebruiker toevoegen" spaties/punten/streepjes uit de naam (en gebruikte
// dát voor het echte interne e-mailadres), terwijl inloggen dat niet deed — een gebruikersnaam
// met zo'n teken kreeg daardoor bij het inloggen een ANDER e-mailadres dan waarmee het account
// is aangemaakt, wat altijd tot "auth/invalid-credential" leidde. Nu gebruiken beide dezelfde functie.
function normaliseerUsername(username) {
  return (username || '').toString().trim().toLowerCase()
    .replace(/@borsa\.intern$/, '')
    .replace(/[^a-z0-9_]/g, '');
}

function usernameToEmail(username) {
  return normaliseerUsername(username) + '@borsa.intern';
}
...
function renderSidebar() {
  ...
  if (kandidatenZichtbaar) {
    html += `
      <div class="nav-item" onclick="navigateToKandidaten()" data-page="kandidaten">
  ...
  el.innerHTML = `
    ...
    <div style="display:grid;grid-template-columns:1fr 3fr;gap:20px;align-items:start;">
      ...
      <div style="display:flex;flex-direction:column;gap:12px;">
        ...
        <div class="card" onclick="navigateToKandidaten()" style="cursor:pointer;padding:16px;display:flex;align-items:center;gap:12px;border-left:4px solid var(--oranje);margin:0;">
...
function laadHomePlanningPreview() {
  ...
  if (DEMO_MODE) {
    toon(demoStore.planning);
    return;
  }

  // Laad planning + leden (voor het adres van de host) samen
  Promise.all([
    db.collection('planning').get(),
    (demoStore.leden && demoStore.leden.length)
      ? Promise.resolve(null)
      : db.collection('leden').get().catch(() => null)
  ]).then(([planSnap, ledenSnap]) => {
    if (ledenSnap) {
      const leden = [];
      ledenSnap.forEach(doc => leden.push({ ...doc.data(), id: doc.id }));
      demoStore.leden = leden;
    }
    const rijen = [];
    planSnap.forEach(doc => rijen.push({ id: doc.id, ...doc.data() }));
    toon(rijen);
  }).catch(() => {
...
// Zoekt het adres van de host op in de ledenlijst (op naam / voornaam)
function adresVanHost(hostNaam) {
  const zoek = _slug(hostNaam);
  if (!zoek) return '';
  const leden = demoStore.leden || [];
  let m = leden.find(l => _slug(l.naam) === zoek);
  if (!m) m = leden.find(l => {
    const delen = (l.naam || '').trim().split(/\s+/).filter(Boolean);
    return delen.length && _slug(delen[0]) === zoek;
  });
  return (m && m.adres) ? m.adres : '';
}
...
function renderPlanningPreviewRows(rijen) {
  ...
  return rijen.map(r => {
    const adres = adresVanHost(r.hosten);
    return `
    <div style="display:flex;align-items:center;gap:16px;padding:10px 0;border-bottom:1px solid var(--grijs);">
      ...
      <div style="flex:1;">
        <div style="font-size:13px;font-weight:600;">${escapeHtml(r.typeAvond || '—')}</div>
        ${r.hosten ? `<div style="font-size:12px;color:var(--grijs-donker);">Host: ${escapeHtml(r.hosten)}${
          adres ? ' · 📍 ' + escapeHtml(adres) : ' · <span style="color:#bbb;">adres niet bekend</span>'}</div>` : ''}
        ${r.notitie ? `<div style="font-size:12px;color:var(--groen);font-weight:600;">📝 ${escapeHtml(r.notitie)}</div>` : ''}
      </div>
    </div>`;
  }).join('');
}
      ...
      <table class="planning-table">
        <thead>
          <tr>
            ...
            <th style="min-width:200px;">Notitie</th>
            ${isAdmin() ? '<th></th>' : ''}
          </tr>
        </thead>
        <tbody id="planning-tbody">
          ${(isAdmin() ? rows : rows.filter(r => r.datum || r.typeAvond || r.hosten || r.helpen || r.pitchenOud || r.pitchenNieuw || r.notitie)).map(row => planningRowHTML(row, canEditCells)).join('')}
        </tbody>
      </table>
    </div>
...
function planningRowHTML(row, canEditCells) {
  ...
  const tijdVeld = adminOnly
    ? `<input type="time" value="${row.tijd || ''}" onchange="savePlanningCell('${row.id}','tijd',this.value)" style="width:100%;">
       <div style="font-size:10px;color:var(--grijs-donker);margin-top:3px;">tot (alleen agenda)</div>
       <input type="time" value="${row.eindTijd || ''}" onchange="savePlanningCell('${row.id}','eindTijd',this.value)" style="width:100%;">`
  ...
  const notitieVeld = adminOnly
    ? `<input type="text" value="${escapeHtml(row.notitie || '')}" placeholder="Bv. adres, of: eerst bij A, dan B" onchange="savePlanningCell('${row.id}','notitie',this.value)" style="min-width:190px;">`
    : `<span style="font-size:13px;">${escapeHtml(row.notitie || '') || '—'}</span>`;
...
function planningRijNaarVevent(row) {
  ...
  if (row.tijd && /^\d{1,2}:\d{2}$/.test(row.tijd)) {
    ...
    let e;
    if (row.eindTijd && /^\d{1,2}:\d{2}$/.test(row.eindTijd)) {
      const [eh, em] = row.eindTijd.split(':').map(Number);
      e = new Date(d.getFullYear(), d.getMonth(), d.getDate(), eh, em);
      // Eindtijd voor of gelijk aan starttijd -> volgende dag (bv. borrel tot 01:00)
      if (e.getTime() <= new Date(d.getFullYear(), d.getMonth(), d.getDate(), hh, mm).getTime()) {
        e = new Date(d.getFullYear(), d.getMonth(), d.getDate() + 1, eh, em);
      }
    } else {
      e = new Date(d.getFullYear(), d.getMonth(), d.getDate(), hh + 2, mm);
    }
  ...
  if (row.notitie) { desc.push('Notitie: ' + row.notitie); lines.push('LOCATION:' + icsEscape(row.notitie)); }
  if (row.locatie) { desc.push('Locatie: ' + row.locatie); }
...
function savePlanningCell(id, field, value) {
  const adminOnlyFields = ['datum', 'tijd', 'eindTijd', 'typeAvond', 'notitie'];
...
function addPlanningRow() {
  ...
  const row = { id, datum: '', tijd: '', eindTijd: '', typeAvond: '', locatie: '', notitie: '', hosten: '', helpen: '', pitchenOud: '', pitchenNieuw: '' };
...
// Alleen letters/cijfers, kleine letters, accenten weg — voor losse vergelijking
function _slug(s) {
...
function mijnLidRij() {
  const leden = demoStore.leden || [];
  const username = _slug(currentUserData && currentUserData.username);
  const display = _slug(currentUserData && currentUserData.displayName);
  const email = (currentUserData && currentUserData.email || '').toLowerCase().trim();
  const zoek = [username, display].filter(Boolean);

  // 1) Expliciete koppeling (koppelUsername == gebruikersnaam)
  if (username) {
    const m = leden.find(l => _slug(l.koppelUsername) === username);
    if (m) return m;
  }

  // 2) Automatisch matchen op naam / voornaam / voornaam+achternaam / e-mail
  for (const lid of leden) {
    const delen = (lid.naam || '').trim().split(/\s+/).filter(Boolean);
    if (!delen.length) continue;
    const volledig = _slug(delen.join(''));            // "oscarheijsteeg"
    const voornaam = _slug(delen[0]);                  // "oscar"
    const voornaamInit = voornaam + _slug(delen[delen.length - 1]).charAt(0); // "oscarh"
    const varianten = new Set([volledig, voornaam, voornaamInit].filter(Boolean));
    if (zoek.some(z => varianten.has(z))) return lid;
    const lidEmail = (lid.email || '').toLowerCase().trim();
    if (email && lidEmail && email === lidEmail) return lid;
  }
  return null;
}

// Handmatige koppeling als automatisch matchen niet lukt
function koppelMijnLid(lidId) {
  const lid = (demoStore.leden || []).find(l => l.id === lidId);
  const username = (currentUserData && currentUserData.username || '').toLowerCase().trim();
  if (!lid || !username) return;
  lid.koppelUsername = username;
  if (!DEMO_MODE) {
    db.collection('leden').doc(lidId).update({ koppelUsername: username })
      .catch(e => showToast('Koppelen mislukt: ' + e.message, 'error'));
  }
  showToast('Gekoppeld! Je gegevens staan nu bovenaan.');
  navigateTo('leden');
}

function mijnLidKiezerHTML(leden) {
  const opties = leden.slice()
    .sort((a, b) => (a.naam || '').localeCompare(b.naam || ''))
    .map(l => `<option value="${l.id}">${escapeHtml(l.naam || '(naamloos)')}</option>`).join('');
  return `
    <div class="card" style="border-left:4px solid var(--goud);margin-bottom:20px;">
      <div style="font-size:10px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:var(--grijs-donker);margin-bottom:8px;">Jouw gegevens</div>
      <div style="font-size:13px;margin-bottom:10px;">We konden je eigen rij niet automatisch herkennen. Kies hieronder wie jij bent — dat onthoudt de app.</div>
      <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
        <select id="mijn-lid-kiezer" style="border:1.5px solid var(--grijs);border-radius:3px;padding:7px 10px;font-family:'Inter',sans-serif;font-size:13px;min-width:200px;">
          <option value="">— Kies je naam —</option>
          ${opties}
        </select>
        <button class="btn btn-primary btn-sm" onclick="if(document.getElementById('mijn-lid-kiezer').value) koppelMijnLid(document.getElementById('mijn-lid-kiezer').value)">Dit ben ik</button>
      </div>
    </div>`;
}
  ...
  return `
    <div class="card" style="border-left:4px solid var(--goud);margin-bottom:20px;">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
        <div style="font-size:10px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:var(--grijs-donker);">Jouw gegevens</div>
        <button class="btn btn-outline btn-sm" title="Mijn gegevens bewerken" onclick="openMijnLidBewerken('${lid.id}')">✏️ Bewerken</button>
      </div>
...
function openMijnLidBewerken(lidId) {
  const lid = mijnLidRij();
  if (!lid || lid.id !== lidId) { showToast('Je kunt alleen je eigen gegevens bewerken.', 'error'); return; }
  showModal(`
    <h3>✏️ Mijn gegevens</h3>
    <div class="form-group"><label>Naam</label>
      <input type="text" id="ml-naam" value="${escapeHtml(lid.naam || '')}" placeholder="Volledige naam"></div>
    <div class="form-group"><label>Geboortedatum</label>
      <input type="date" id="ml-geboortedatum" value="${escapeHtml(lid.geboortedatum || '')}"></div>
    <div class="form-group"><label>Telefoon</label>
      <input type="text" id="ml-telefoon" value="${escapeHtml(lid.telefoon || '')}" placeholder="06..."></div>
    <div class="form-group"><label>E-mail</label>
      <input type="email" id="ml-email" value="${escapeHtml(lid.email || '')}" placeholder="naam@mail.com"></div>
    <div class="form-group"><label>Adres</label>
      <input type="text" id="ml-adres" value="${escapeHtml(lid.adres || '')}" placeholder="Straat 1, 1234AB"></div>
    <div class="form-group"><label>Hosten</label>
      <select id="ml-hosten">
    ...
    <div class="form-group"><label>Dieet / allergie</label>
      <input type="text" id="ml-dieet" value="${escapeHtml(lid.dieet || '')}" placeholder="-"></div>
    <div class="modal-footer">
      <button class="btn btn-outline btn-sm" onclick="closeModal()">Annuleren</button>
      <button class="btn btn-primary btn-sm" onclick="saveMijnLid('${lid.id}')">Opslaan</button>
    </div>
  `);
}

function saveMijnLid(lidId) {
  const lid = mijnLidRij();
  if (!lid || lid.id !== lidId) { showToast('Je kunt alleen je eigen gegevens bewerken.', 'error'); return; }

  const updates = {
    naam: document.getElementById('ml-naam').value.trim(),
    geboortedatum: document.getElementById('ml-geboortedatum').value,
    ...
    hosten: document.getElementById('ml-hosten').value,
    dieet: document.getElementById('ml-dieet').value.trim()
  };
  Object.assign(lid, updates);

  if (!DEMO_MODE) {
    db.collection('leden').doc(lidId).update(updates)
      .then(() => { showToast('Je gegevens zijn opgeslagen!'); })
      .catch(e => showToast('Opslaan mislukt: ' + e.message, 'error'));
  } else {
    showToast('Je gegevens zijn opgeslagen!');
  }
  closeModal();
  navigateTo('leden');
}
...
function vulLedenOverzicht(leden) {
  ...
  if (kaartEl) {
    kaartEl.innerHTML = mijnLid
      ? mijnLidCardHTML(mijnLid)
      : (!isAdmin() && leden.length ? mijnLidKiezerHTML(leden) : '');
  }
...
function renderKandidaten(el) {
  ...
  el.innerHTML = `
    <div class="page-header">
      ...
      <div style="display:flex;gap:12px;align-items:center;flex-wrap:wrap;">
        ${isAdmin() ? `
          ...
          <button class="btn btn-outline btn-sm" onclick="openAanwezigheidModal()">🗓️ Aanwezigheid</button>
    ...
    <div id="kandidaten-content"><div class="loading">Even laden...</div></div>
  `;
  laadPotis();
}

// Sidebar/homepage-links gaan hier altijd doorheen zodat je bij een verse
// bezoek aan de pagina het keuzescherm ziet; interne refreshes (bv. na opslaan)
// gebruiken gewoon navigateTo('kandidaten') en behouden de huidige weergave.
function navigateToKandidaten() {
  kandidatenModus = null;
  navigateTo('kandidaten');
}

// Uid -> weergavenaam, nodig om bij "wie heeft gestemd" namen i.p.v. uid's te tonen
function laadGebruikersMap() {
  if (demoStore.gebruikersMap) return Promise.resolve(demoStore.gebruikersMap);
  if (DEMO_MODE) {
    const map = {};
    Object.values(demoStore.users).forEach(u => { map[u.uid] = u.displayName; map[u.username] = u.displayName; });
    demoStore.gebruikersMap = map;
    return Promise.resolve(map);
  }
  return db.collection('users').get().then(snap => {
    const map = {};
    snap.forEach(doc => { const d = doc.data(); map[doc.id] = d.displayName || d.username || doc.id; });
    demoStore.gebruikersMap = map;
    return map;
  }).catch(() => ({}));
}

// ─── WEERGAVE-MODUS: keuze / swipe / grid ─────────────────────────────────────
let kandidatenModus = null; // null (= keuzescherm) | 'swipe' | 'grid'

function laadPotis() {
  const contentEl = document.getElementById('kandidaten-content');
  if (!contentEl) return;

  if (DEMO_MODE) {
    if (isAdmin()) laadGebruikersMap();
    renderKandidatenContent();
    return;
  }

  // Laad poti's (en, voor de admin, de gebruikersnamen voor "wie heeft gestemd") uit Firestore
  Promise.all([
    db.collection('kandidaten').get(),
    isAdmin() ? laadGebruikersMap() : Promise.resolve({})
  ]).then(([snap]) => {
    ...
    if (nogTeLaden.length > 0) {
      ...
      db.collectionGroup('comments').get().then(csnap => {
        ...
        // Alleen relevant in grid-modus — deze elementen bestaan enkel dan
    ...
    const el = document.getElementById('kandidaten-content');
    if (el) el.innerHTML = `<div class="alert alert-error">Fout: ${e.message}</div>`;
  });
}

function kiesKandidatenModus(modus) {
  kandidatenModus = modus;
  renderKandidatenContent();
}

function renderKandidatenContent() {
  const el = document.getElementById('kandidaten-content');
  if (!el) return;

  if (demoStore.kandidaten.length === 0) {
    el.innerHTML = `<div class="empty-state"><div class="empty-icon">👥</div><p>Nog geen poti's toegevoegd.</p></div>`;
    return;
  }

  if (kandidatenModus === 'swipe') {
    renderSwipeModus(el);
  } else if (kandidatenModus === 'grid') {
    el.innerHTML = `
      <div style="margin-bottom:16px;"><button class="btn btn-outline btn-sm" onclick="kiesKandidatenModus(null)">← Terug</button></div>
      <div class="candidates-grid">${demoStore.kandidaten.map(k => kandidaatCardHTML(k)).join('')}</div>`;
  } else {
    el.innerHTML = `
      <div style="display:flex;gap:20px;flex-wrap:wrap;justify-content:center;padding:40px 20px;">
        <div class="card" onclick="kiesKandidatenModus('swipe')" style="cursor:pointer;text-align:center;padding:32px 24px;flex:1;min-width:220px;max-width:280px;">
          <div style="font-size:40px;">🔥</div>
          <div style="font-weight:700;font-size:16px;color:var(--groen);margin-top:8px;">Mening geven over poti's</div>
          <div style="font-size:12px;color:var(--grijs-donker);margin-top:6px;">Eén voor één swipen of klikken door iedereen die je nog niet beoordeeld hebt.</div>
        </div>
        <div class="card" onclick="kiesKandidatenModus('grid')" style="cursor:pointer;text-align:center;padding:32px 24px;flex:1;min-width:220px;max-width:280px;">
          <div style="font-size:40px;">🗂️</div>
          <div style="font-weight:700;font-size:16px;color:var(--groen);margin-top:8px;">Individuele poti bekijken</div>
          <div style="font-size:12px;color:var(--grijs-donker);margin-top:6px;">Blader zelf door de volledige lijst en reageer gericht op iemand.</div>
        </div>
      </div>`;
  }
}
...
let swipeQueue = [];
let swipeIndex = 0;
let swipeLaatsteOptie = null;

function renderSwipeModus(el) {
  const mijnUid = (currentUser && currentUser.uid) || currentUserData.uid || currentUserData.username;
  swipeQueue = demoStore.kandidaten.filter(k => !k.stemmen || !k.stemmen[mijnUid]);
  swipeIndex = 0;
  renderSwipeKaart();
}

function herstartSwipe() {
  swipeQueue = demoStore.kandidaten.slice();
  swipeIndex = 0;
  renderSwipeKaart();
}

function renderSwipeKaart() {
  const el = document.getElementById('kandidaten-content');
  if (!el) return;

  const terugKnop = `<div style="margin-bottom:12px;"><button class="btn btn-outline btn-sm" onclick="kiesKandidatenModus(null)">← Terug</button></div>`;

  if (swipeIndex >= swipeQueue.length) {
    el.innerHTML = `${terugKnop}
      <div class="empty-state">
        <div class="empty-icon">🎉</div>
        <p>Je hebt iedereen beoordeeld!</p>
        <div style="display:flex;gap:10px;justify-content:center;margin-top:12px;flex-wrap:wrap;">
          <button class="btn btn-outline btn-sm" onclick="kiesKandidatenModus('grid')">Bekijk overzicht</button>
          <button class="btn btn-primary btn-sm" onclick="herstartSwipe()">🔁 Opnieuw beginnen</button>
        </div>
      </div>`;
    return;
  }

  const k = swipeQueue[swipeIndex];
  const geslacht = k.geslacht === 'M' ? '♂' : k.geslacht === 'V' ? '♀' : '⚧';

  el.innerHTML = `
    ${terugKnop}
    <div style="text-align:center;font-size:12px;color:var(--grijs-donker);margin-bottom:10px;">${swipeIndex + 1} van ${swipeQueue.length}</div>
    <div style="display:flex;justify-content:center;">
      <div id="swipe-card" class="candidate-card" style="width:100%;max-width:320px;position:relative;touch-action:pan-y;user-select:none;">
        <div id="swipe-overlay-leuk" style="position:absolute;top:16px;left:16px;padding:6px 14px;border:3px solid #27ae60;color:#27ae60;font-weight:700;border-radius:4px;transform:rotate(-15deg);opacity:0;font-size:14px;z-index:2;pointer-events:none;">LEUK</div>
        <div id="swipe-overlay-nietleuk" style="position:absolute;top:16px;right:16px;padding:6px 14px;border:3px solid #e74c3c;color:#e74c3c;font-weight:700;border-radius:4px;transform:rotate(15deg);opacity:0;font-size:14px;z-index:2;pointer-events:none;">NIET LEUK</div>
        <div id="swipe-big-emoji" style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;font-size:100px;opacity:0;transition:opacity 0.15s ease;pointer-events:none;z-index:3;filter:drop-shadow(0 4px 10px rgba(0,0,0,0.3));"></div>
        <div class="candidate-photo">
          ${k.fotoUrl ? `<img src="${k.fotoUrl}" alt="${escapeHtml(k.naam)}">` : '👤'}
        </div>
        <div class="candidate-body">
          <div class="candidate-name">${escapeHtml(k.naam)} ${geslacht}</div>
          <div class="candidate-meta">Leeftijd: <span>${escapeHtml(String(k.leeftijd || '—'))}</span></div>
          <div class="candidate-meta">Studie: <span>${escapeHtml(k.studie || '—')}</span></div>
        </div>
      </div>
    </div>
    <div style="display:flex;justify-content:center;gap:16px;margin-top:20px;">
      <button class="btn btn-danger" style="min-width:130px;" onclick="swipeStem('niet-leuk')">👎 Niet leuk</button>
      <button class="btn btn-primary" style="min-width:130px;background:var(--groen);" onclick="swipeStem('leuk')">👍 Leuk</button>
    </div>
    <div style="display:flex;justify-content:center;gap:12px;margin-top:12px;">
      <button class="btn btn-outline btn-sm" onclick="swipeStem('ken-ik-niet')">Ken ik niet</button>
      <button class="btn btn-outline btn-sm" onclick="swipeStem('niet-genoeg')">Niet genoeg gesproken</button>
    </div>
  `;
...
function swipeStem(optieKey) {
  if (optieKey === 'leuk' || optieKey === 'niet-leuk') {
    // Knop-klik: kaart is nog niet aan het slepen, dus animeer 'm zelf weg
    animeerSwipeUitkomst(optieKey === 'leuk' ? 1 : -1, optieKey);
  } else {
    voltooiSwipeKeuze(optieKey);
  }
}

// Laat de kaart wegvliegen + het grote emoji-icoon oplichten, en rondt daarna de keuze af
function animeerSwipeUitkomst(richting, optieKey) {
  const card = document.getElementById('swipe-card');
  if (card) {
    card.style.transition = 'transform 0.25s ease';
    card.style.transform = `translateX(${richting * 600}px) rotate(${richting * 20}deg)`;
  }
  const emojiEl = document.getElementById('swipe-big-emoji');
  if (emojiEl) {
    emojiEl.textContent = optieKey === 'leuk' ? '🫶' : '❌';
    emojiEl.style.opacity = '1';
  }
  setTimeout(() => voltooiSwipeKeuze(optieKey), 260);
}

function voltooiSwipeKeuze(optieKey) {
  const k = swipeQueue[swipeIndex];
  if (!k) return;
  setStem(k.id, optieKey);
  swipeLaatsteOptie = optieKey;
  renderSwipeNotitieStap(k);
}

function renderSwipeNotitieStap(k) {
  const el = document.getElementById('kandidaten-content');
  if (!el) return;
  const label = (STEM_OPTIES.find(o => o.key === swipeLaatsteOptie) || {}).label || '';

  el.innerHTML = `
    <div style="margin-bottom:12px;"><button class="btn btn-outline btn-sm" onclick="kiesKandidatenModus(null)">← Terug</button></div>
    <div style="text-align:center;font-size:12px;color:var(--grijs-donker);margin-bottom:10px;">${swipeIndex + 1} van ${swipeQueue.length}</div>
    <div style="display:flex;justify-content:center;">
      <div class="candidate-card" style="width:100%;max-width:320px;padding:20px;">
        <div style="text-align:center;font-weight:700;color:var(--groen);margin-bottom:4px;">${escapeHtml(k.naam)}</div>
        <div style="text-align:center;font-size:13px;color:var(--grijs-donker);margin-bottom:16px;">Jouw stem: <strong>${escapeHtml(label)}</strong></div>
        <div class="form-group">
          <label>Opmerking (optioneel)</label>
          <textarea id="swipe-notitie" rows="3" placeholder="Jouw mening..."></textarea>
        </div>
        <button class="btn btn-primary" style="width:100%;" onclick="swipeVolgende('${k.id}')">Volgende →</button>
      </div>
    </div>`;

  document.getElementById('swipe-notitie')?.focus();
}

function swipeVolgende(kandidaatId) {
  const tekst = document.getElementById('swipe-notitie')?.value.trim();
  if (tekst) {
    const comment = {
      id: 'c_' + Date.now(),
    ...
    if (!DEMO_MODE) {
      db.collection('kandidaten').doc(kandidaatId).collection('comments').add(comment)
        .catch(e => showToast('Fout bij opslaan opmerking: ' + e.message, 'error'));
    }
  }
  swipeIndex++;
  renderSwipeKaart();
...
// Sleep-gedrag voor de swipe-kaart. De move/end-listeners staan één keer globaal
// geregistreerd (i.p.v. per kaart) om te voorkomen dat ze bij elke render stapelen.
let swipeDrag = { card: null, startX: 0, currentX: 0, active: false, overlayLeuk: null, overlayNietLeuk: null };

function initSwipeDrag() {
  const card = document.getElementById('swipe-card');
  if (!card) return;
  swipeDrag.overlayLeuk = document.getElementById('swipe-overlay-leuk');
  swipeDrag.overlayNietLeuk = document.getElementById('swipe-overlay-nietleuk');
...
function swipeDragMove(x) {
  const card = swipeDrag.card;
  if (!card) return;
  const dx = x - swipeDrag.startX;
  swipeDrag.currentX = dx;
  card.style.transform = `translateX(${dx}px) rotate(${dx / 20}deg)`;
  if (swipeDrag.overlayLeuk) swipeDrag.overlayLeuk.style.opacity = Math.max(0, Math.min(1, dx / 80));
  if (swipeDrag.overlayNietLeuk) swipeDrag.overlayNietLeuk.style.opacity = Math.max(0, Math.min(1, -dx / 80));
}
...
function swipeDragEnd() {
  ...
  card.style.transition = 'transform 0.25s ease';
  const drempel = 100;
  const dx = swipeDrag.currentX;
  if (dx > drempel) {
    card.style.transform = 'translateX(600px) rotate(20deg)';
    const emojiEl = document.getElementById('swipe-big-emoji');
    if (emojiEl) { emojiEl.textContent = '🫶'; emojiEl.style.opacity = '1'; }
    setTimeout(() => voltooiSwipeKeuze('leuk'), 220);
  } else if (dx < -drempel) {
    card.style.transform = 'translateX(-600px) rotate(-20deg)';
    const emojiEl = document.getElementById('swipe-big-emoji');
    if (emojiEl) { emojiEl.textContent = '❌'; emojiEl.style.opacity = '1'; }
    setTimeout(() => voltooiSwipeKeuze('niet-leuk'), 220);
  } else {
    card.style.transform = '';
    if (swipeDrag.overlayLeuk) swipeDrag.overlayLeuk.style.opacity = 0;
    if (swipeDrag.overlayNietLeuk) swipeDrag.overlayNietLeuk.style.opacity = 0;
  }
  swipeDrag.card = null;
}

document.addEventListener('touchmove', e => { if (swipeDrag.active) swipeDragMove(e.touches[0].clientX); }, { passive: true });
document.addEventListener('touchend', () => { if (swipeDrag.active) swipeDragEnd(); });
document.addEventListener('mousemove', e => { if (swipeDrag.active) swipeDragMove(e.clientX); });
document.addEventListener('mouseup', () => { if (swipeDrag.active) swipeDragEnd(); });
...
function kandidaatCardHTML(k) {
  ...
  // Stemverdeling in % — zichtbaar voor iedereen
  const totaalStemmen = Object.keys(k.stemmen).length;
  const stemSummaryHTML = STEM_OPTIES.map(o => {
  ...
  // Reacties zijn voor iedereen zichtbaar (ook rol Lid)
  const meningenSectie = `
  ...
  return `
    <div class="candidate-card" id="kcard-${k.id}">
      ...
      <div class="candidate-body">
        ...
        <div class="stem-summary" id="stemsummary-${k.id}">${stemSummaryHTML}</div>
        ${isAdmin() ? stemVerloopHTML(k) : ''}
        ${isAdmin() ? stemLijstDetailsHTML(k) : ''}
      </div>
      ${meningenSectie}
    </div>`;
}

const STEM_KLEUR = { 'leuk': '#28a745', 'niet-leuk': '#dc3545', 'ken-ik-niet': '#5b6b7a', 'niet-genoeg': '#c99700' };
...
// Verdeling van stemmen per dag, opnieuw afgespeeld uit k.stemLog
function stemVerdelingPerDag(k) {
  const log = (Array.isArray(k.stemLog) ? k.stemLog : [])
    .filter(e => e && e.t).slice().sort((a, b) => a.t - b.t);

  if (log.length === 0) {
    const stemmen = k.stemmen || {};
    if (!Object.keys(stemmen).length) return null;
    const telling = {};
    STEM_OPTIES.forEach(o => telling[o.key] = 0);
    Object.values(stemmen).forEach(s => { if (telling[s] !== undefined) telling[s]++; });
    return [{ dag: _dagStr(Date.now()), telling }];
  }

  const dagen = Array.from(new Set(log.map(e => _dagStr(e.t))));
  const vandaag = _dagStr(Date.now());
  if (!dagen.includes(vandaag)) dagen.push(vandaag);
  dagen.sort();

  return dagen.map(dag => {
    const grens = new Date(dag + 'T23:59:59').getTime();
    const stand = {};
    log.forEach(e => { if (e.t <= grens) stand[e.u] = e.s; });
    const telling = {};
    STEM_OPTIES.forEach(o => telling[o.key] = 0);
    Object.values(stand).forEach(s => { if (telling[s] !== undefined) telling[s]++; });
    return { dag, telling };
  });
}

function stemGrafiekSVG(k) {
  const serie = stemVerdelingPerDag(k);
  if (!serie || serie.length === 0) {
    return '<div style="font-size:12px;color:var(--grijs-donker);padding:6px 0;">Nog geen stemmen om te tonen.</div>';
  }

  const W = 340, H = 150, mL = 24, mR = 8, mT = 10, mB = 26;
  const pW = W - mL - mR, pH = H - mT - mB;
  const n = serie.length;
  let maxY = 1;
  serie.forEach(p => STEM_OPTIES.forEach(o => { if (p.telling[o.key] > maxY) maxY = p.telling[o.key]; }));
  ...
  const assen = `
    ...
    <text x="${mL - 5}" y="${y(maxY) + 4}" text-anchor="end" font-size="9" fill="#999">${maxY}</text>
    <text x="${mL - 5}" y="${mT + pH + 4}" text-anchor="end" font-size="9" fill="#999">0</text>`;

  const lijnen = STEM_OPTIES.map(o => {
    const kleur = STEM_KLEUR[o.key] || '#888';
    const pts = serie.map((p, i) => `${x(i).toFixed(1)},${y(p.telling[o.key]).toFixed(1)}`).join(' ');
    const dots = serie.map((p, i) => `<circle cx="${x(i).toFixed(1)}" cy="${y(p.telling[o.key]).toFixed(1)}" r="2.5" fill="${kleur}"/>`).join('');
    return `<polyline points="${pts}" fill="none" stroke="${kleur}" stroke-width="2"/>${dots}`;
  }).join('');

  const xLabels = serie.map((p, i) => {
    if (n > 4 && i % Math.ceil(n / 4) !== 0 && i !== n - 1) return '';
    const d = p.dag.split('-');
    return `<text x="${x(i).toFixed(1)}" y="${mT + pH + 16}" text-anchor="middle" font-size="9" fill="#999">${d[2]}-${d[1]}</text>`;
  }).join('');

  const legenda = STEM_OPTIES.map(o =>
    `<span style="display:inline-flex;align-items:center;gap:4px;margin-right:10px;font-size:11px;">
       <span style="width:10px;height:10px;border-radius:2px;background:${STEM_KLEUR[o.key]};display:inline-block;"></span>${o.label}
     </span>`).join('');

  return `
    <svg viewBox="0 0 ${W} ${H}" style="width:100%;max-width:${W}px;height:auto;">
      ${assen}${lijnen}${xLabels}
    </svg>
    <div style="margin-top:4px;line-height:1.9;">${legenda}</div>`;
}

function stemVerloopHTML(k) {
  return `
    <details style="margin-top:10px;">
      <summary style="cursor:pointer;font-size:12px;font-weight:700;color:var(--groen);">📈 Stemverloop</summary>
      <div id="stemgrafiek-${k.id}" style="margin-top:6px;">${stemGrafiekSVG(k)}</div>
    </details>`;
}

// Naam bij een uid, met terugval op de ruwe uid als er geen match is
function stemNaam(uid) {
  const map = demoStore.gebruikersMap || {};
  if (map[uid]) return map[uid];
  // Geen gebruiker meer met dit uid (bv. account is verwijderd en opnieuw aangemaakt) —
  // toon dat duidelijk in plaats van de kale, onherkenbare uid
  return 'Onbekende gebruiker (verwijderd account)';
}

// Wie heeft (nog) niet gestemd — lijst met naam + gekozen optie, admin-only
function stemLijstHTML(k) {
  const stemmen = k.stemmen || {};
  const uids = Object.keys(stemmen);
  const optieLabel = key => (STEM_OPTIES.find(o => o.key === key) || {}).label || key;
  const kleur = key => STEM_KLEUR[key] || '#888';

  const gestemdHTML = uids.length === 0
    ? '<div style="font-size:12px;color:var(--grijs-donker);padding:4px 0;">Nog niemand heeft gestemd.</div>'
    : uids.map(uid => ({ naam: stemNaam(uid), stem: stemmen[uid] }))
        .sort((a, b) => a.naam.localeCompare(b.naam))
        .map(r => `
          <div style="display:flex;justify-content:space-between;align-items:center;font-size:12px;padding:3px 0;border-bottom:1px solid var(--grijs-licht);">
            <span>${escapeHtml(r.naam)}</span>
            <span style="font-weight:700;color:${kleur(r.stem)};">${escapeHtml(optieLabel(r.stem))}</span>
          </div>`).join('');

  // Leden die (voor zover bekend) nog niet gestemd hebben
  const alleGebruikers = Object.entries(demoStore.gebruikersMap || {});
  const nogNiet = Array.from(new Set(alleGebruikers.filter(([uid]) => !(uid in stemmen)).map(([, naam]) => naam)));
  const nogNietHTML = nogNiet.length
    ? `<div style="margin-top:10px;font-size:11px;color:var(--grijs-donker);">
         <strong>Nog niet gestemd (${nogNiet.length}):</strong> ${nogNiet.map(escapeHtml).join(', ')}
       </div>`
    : '';

  return `<div style="margin-top:4px;">${gestemdHTML}</div>${nogNietHTML}`;
}

function stemLijstDetailsHTML(k) {
  const aantal = Object.keys(k.stemmen || {}).length;
  return `
    <details style="margin-top:6px;">
      <summary id="stemlijst-titel-${k.id}" style="cursor:pointer;font-size:12px;font-weight:700;color:var(--groen);">👥 Wie heeft gestemd (${aantal})</summary>
      <div id="stemlijst-${k.id}" style="margin-top:6px;">${stemLijstHTML(k)}</div>
    </details>`;
}
...
// ─── AANWEZIGHEID PER INTRO-AVOND (admin) ─────────────────────────────────────

function openAanwezigheidModal() {
  if (!isAdmin()) return;
  const datums = introActiviteitDatums();

  showModal(`
    <h3>🗓️ Aanwezigheid intro-avond</h3>
    <div class="form-group">
      <label>Avond</label>
      <select id="aanw-datum" onchange="renderAanwezigheidLijst(this.value)">
        <option value="">— Kies een avond —</option>
        ${datums.map(d => {
          const rij = (demoStore.planning || []).find(r => r.datum === d);
          const label = new Date(d).toLocaleDateString('nl-NL', { weekday: 'long', day: 'numeric', month: 'long' });
          return `<option value="${d}">${label}${rij && rij.typeAvond ? ' — ' + escapeHtml(rij.typeAvond) : ''}</option>`;
        }).join('')}
      </select>
    </div>
    <div id="aanwezigheid-lijst" style="margin-top:12px;">
      ${datums.length === 0
        ? `<p style="font-size:12px;color:var(--grijs-donker);">Nog geen intro-activiteiten in de planning (type "Intro Activiteit" of "Introborrel").</p>`
        : `<p style="font-size:12px;color:var(--grijs-donker);">Kies eerst een avond.</p>`}
    </div>
    <div class="modal-footer">
      <button class="btn btn-outline btn-sm" onclick="closeModal()">Annuleren</button>
      <button class="btn btn-primary btn-sm" onclick="slaAanwezigheidOp()">Opslaan</button>
    </div>
  `);
...
function renderAanwezigheidLijst(datum) {
  const el = document.getElementById('aanwezigheid-lijst');
  if (!el) return;
  if (!datum) { el.innerHTML = `<p style="font-size:12px;color:var(--grijs-donker);">Kies eerst een avond.</p>`; return; }

  const potis = demoStore.kandidaten || [];
  if (potis.length === 0) {
    el.innerHTML = `<p style="font-size:12px;color:var(--grijs-donker);">Nog geen poti's toegevoegd.</p>`;
    return;
  }

  el.innerHTML = `
    <div style="display:flex;font-size:10px;font-weight:700;letter-spacing:1px;text-transform:uppercase;color:var(--grijs-donker);padding:6px 0;border-bottom:1px solid var(--grijs);">
      <div style="min-width:180px;">Aanwezig</div>
      <div style="flex:1;">Opmerking (optioneel)</div>
    </div>
    <div style="max-height:360px;overflow-y:auto;">
      ${potis.map(k => {
        const aanwezig = Array.isArray(k.introDatums) && k.introDatums.includes(datum);
        return `
        <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--grijs);">
          <label style="display:flex;align-items:center;gap:8px;min-width:180px;cursor:pointer;">
            <input type="checkbox" class="aanw-check" data-kid="${k.id}" ${aanwezig ? 'checked' : ''}>
            <span style="font-weight:600;font-size:13px;">${escapeHtml(k.naam)}</span>
          </label>
          <input type="text" class="aanw-comment" data-kid="${k.id}" placeholder="Bv. leuk gesprek gehad over..."
            style="flex:1;padding:6px 10px;border:1.5px solid var(--grijs);border-radius:3px;font-family:'Inter',sans-serif;font-size:12px;">
        </div>`;
      }).join('')}
    </div>`;
}

function slaAanwezigheidOp() {
  if (!isAdmin()) return;
  const datum = document.getElementById('aanw-datum')?.value;
  if (!datum) { showToast('Kies eerst een avond.', 'error'); return; }

  const checks = document.querySelectorAll('.aanw-check');
  if (checks.length === 0) { showToast('Geen poti\'s om aanwezigheid voor te registreren.', 'error'); return; }

  const opmerkingPerKid = {};
  document.querySelectorAll('.aanw-comment').forEach(inp => {
    const tekst = inp.value.trim();
    if (tekst) opmerkingPerKid[inp.dataset.kid] = tekst;
  });

  const datumLabel = new Date(datum).toLocaleDateString('nl-NL', { day: 'numeric', month: 'short' });

  checks.forEach(chk => {
    const kid = chk.dataset.kid;
    const k = demoStore.kandidaten.find(x => x.id === kid);
    if (!k) return;

    if (!Array.isArray(k.introDatums)) k.introDatums = [];
    const idx = k.introDatums.indexOf(datum);
    if (chk.checked && idx === -1) k.introDatums.push(datum);
    if (!chk.checked && idx !== -1) k.introDatums.splice(idx, 1);

    if (!DEMO_MODE) {
      db.collection('kandidaten').doc(kid).update({ introDatums: k.introDatums })
        .catch(e => showToast('Fout bij opslaan aanwezigheid: ' + e.message, 'error'));
    }

    const opmerking = opmerkingPerKid[kid];
    if (opmerking) {
      const comment = {
        id: 'c_' + Date.now() + '_' + kid,
        author: currentUserData.displayName,
        rol: currentUserData.rol,
        tekst: `🗓️ ${datumLabel}: ${opmerking}`,
        datum: new Date().toLocaleDateString('nl-NL')
      };
      if (!demoStore.comments[kid]) demoStore.comments[kid] = [];
      demoStore.comments[kid].push(comment);
      if (!DEMO_MODE) {
        db.collection('kandidaten').doc(kid).collection('comments').add(comment)
          .catch(e => showToast('Fout bij opslaan opmerking: ' + e.message, 'error'));
      }
    }
  });
...
function setStem(kandidaatId, stemKey) {
  ...
  // Leg de wijziging vast met tijdstip voor het stemverloop
  if (!Array.isArray(k.stemLog)) k.stemLog = [];
  k.stemLog.push({ u: mijnUid, s: k.stemmen[mijnUid] || '', t: Date.now() });

  if (!DEMO_MODE) {
    db.collection('kandidaten').doc(kandidaatId).update({ stemmen: k.stemmen, stemLog: k.stemLog });
  }
  ...
  if (summary) {
    ...
    STEM_OPTIES.forEach(o => stemTellingen[o.key] = 0);
    Object.values(k.stemmen).forEach(s => { if (stemTellingen[s] !== undefined) stemTellingen[s]++; });
    const totaal = Object.keys(k.stemmen).length;
    summary.innerHTML = STEM_OPTIES.map(o => {
      const pct = totaal > 0 ? Math.round((stemTellingen[o.key] / totaal) * 100) : 0;
      return `<span title="${stemTellingen[o.key]} stem(men)">${o.label}: <strong>${pct}%</strong></span>`;
    }).join('') + `<span style="color:var(--grijs-donker);margin-left:4px;">(${totaal} stem${totaal !== 1 ? 'men' : ''})</span>`;
  }

  const graf = document.getElementById('stemgrafiek-' + kandidaatId);
  if (graf) graf.innerHTML = stemGrafiekSVG(k);

  const lijst = document.getElementById('stemlijst-' + kandidaatId);
  if (lijst) lijst.innerHTML = stemLijstHTML(k);
  const lijstTitel = document.getElementById('stemlijst-titel-' + kandidaatId);
  if (lijstTitel) lijstTitel.textContent = `👥 Wie heeft gestemd (${Object.keys(k.stemmen).length})`;
}
...
function renderGebruikersbeheer(el) {
  el.innerHTML = `
    ...
    <div class="alert alert-info" style="margin-top:16px;">
      ...
      Gebruik "Wijzig wachtwoord" om een nieuw wachtwoord in te stellen — vul daarbij zo mogelijk ook het
      <em>huidige</em> wachtwoord in, anders wijzigt alleen de weergave hier en blijft het echte inlog-wachtwoord
      het oude (dit was de oorzaak van "auth/invalid-credential" bij een deel van de leden).
    ...
    <div class="alert alert-error" style="margin-top:12px;font-size:12px;">
      <strong>⚠️ Account echt kapot (kan niet meer inloggen, huidig wachtwoord onbekend)?</strong>
      Verwijder de gebruiker hier met "Verwijderen", verwijder daarna hetzelfde account (zelfde interne
      e-mailadres, bijv. <code>gebruikersnaam@borsa.intern</code>) ook in de
      <a href="https://console.firebase.google.com/project/${firebaseConfig.projectId}/authentication/users" target="_blank">Firebase Console → Authentication → Users</a>,
      en maak het account daarna opnieuw aan via "+ Gebruiker toevoegen". Alleen zo verwijderd blijft de
      gebruikersnaam beschikbaar en werkt het nieuwe wachtwoord meteen.
    </div>
  `;
...
function openWachtwoordModal(uid, naam, email) {
  showModal(`
    ...
    <p style="color:var(--grijs-donker);font-size:13px;margin-bottom:12px;">Nieuw wachtwoord instellen voor <strong>${naam}</strong></p>
    <div class="alert alert-info" style="font-size:12px;margin-bottom:16px;">
      <strong>Belangrijk:</strong> Firebase staat alleen toe dat het écht in te loggen wachtwoord wordt
      gewijzigd als het <em>huidige</em> wachtwoord bekend is. Vul dat hieronder in als je het weet.
      Laat je het leeg, dan wordt alleen de weergave hier aangepast — inloggen blijft dan met het
      <u>oude</u> wachtwoord, en dit is precies de oorzaak van "auth/invalid-credential" bij mensen
      van wie het wachtwoord ooit zo is "gewijzigd".
    </div>
    <div class="form-group">
      <label>Huidig wachtwoord (indien bekend)</label>
      <input type="text" id="oud-pw-input" placeholder="Laat leeg als onbekend" autocomplete="off">
    </div>
    <div class="form-group">
...
function saveNieuwWachtwoord(uid, email) {
  ...
  const oudPw = document.getElementById('oud-pw-input').value;
  ...
  db.collection('users').doc(uid).update({ cachedPw: nieuwPw }).then(() => {
    const valueEl = document.getElementById('pw-value-' + uid);
    if (valueEl) valueEl.dataset.pw = nieuwPw;

    if (!oudPw) {
      // Zonder huidig wachtwoord kan het ECHTE Firebase Auth-wachtwoord niet gewijzigd worden.
      // Alleen de weergave hier is aangepast.
      closeModal();
      showToast('Let op: alleen de weergave is aangepast — inloggen werkt nog met het oude wachtwoord! Vul bij een volgende poging het huidige wachtwoord in, of verwijder dit account en maak het opnieuw aan.', 'error', 9000);
      return;
    }

    // Met het huidige wachtwoord kunnen we écht inloggen als deze gebruiker en het wachtwoord wijzigen
    const secondaryApp = firebase.apps.find(a => a.name === 'Secondary')
      || firebase.initializeApp(firebaseConfig, 'Secondary');
    const secondaryAuth = secondaryApp.auth();

    secondaryAuth.signInWithEmailAndPassword(email, oudPw)
      .then(cred => cred.user.updatePassword(nieuwPw))
      .then(() => secondaryAuth.signOut())
      .then(() => {
        closeModal();
        showToast('Wachtwoord écht gewijzigd — inloggen werkt nu meteen met het nieuwe wachtwoord!');
      })
      .catch(e => {
        secondaryAuth.signOut().catch(() => {});
        closeModal();
        showToast('Het opgegeven huidige wachtwoord klopt niet, dus het echte inlog-wachtwoord is NIET gewijzigd. Verwijder dit account en maak het opnieuw aan.', 'error', 9000);
      });
  }).catch(e => {
  ...
  showModal(`
    ...
    <div class="form-group">
      ...
      <div style="display:flex;align-items:center;gap:0;">
        <input type="text" id="nu-username" placeholder="jandevries" autocapitalize="none" style="border-radius:3px 0 0 3px;flex:1;" oninput="document.getElementById('nu-username-preview').textContent = normaliseerUsername(this.value) || '—';">
      ...
      <p style="font-size:11px;color:var(--grijs-donker);margin-top:4px;">Spaties, punten en streepjes worden verwijderd. Inloggen kan straks met: <strong id="nu-username-preview">—</strong></p>
    </div>
...
function saveNieuweGebruiker() {
  ...
  const username = normaliseerUsername(document.getElementById('nu-username').value);
