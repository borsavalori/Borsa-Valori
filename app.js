  ...
  return rijen.map(r => `
    <div style="display:flex;align-items:center;gap:16px;padding:10px 0;border-bottom:1px solid var(--grijs);">
      ...
      <div style="flex:1;">
        ...
        ${(r.hosten || r.notitie) ? `<div style="font-size:12px;color:var(--grijs-donker);">${r.hosten ? 'Host: ' + escapeHtml(r.hosten) : ''}${r.hosten && r.notitie ? '&nbsp;&nbsp;·&nbsp;&nbsp;' : ''}${r.notitie ? '📍 ' + escapeHtml(r.notitie) : ''}</div>` : ''}
      </div>
    </div>
  `).join('');
}
