  ...
  if (swipeIndex >= swipeQueue.length) {
    el.innerHTML = `${terugKnop}
      <div class="empty-state">
        ...
        <button class="btn btn-outline btn-sm" style="margin-top:12px;" onclick="kiesKandidatenModus('grid')">Bekijk overzicht</button>
      </div>`;
    return;
  }
...
function swipeDragEnd() {
  ...
  if (dx > drempel) {
    ...
    setTimeout(() => swipeStem('leuk'), 150);
  } else if (dx < -drempel) {
    card.style.transform = 'translateX(-600px) rotate(-20deg)';
    setTimeout(() => swipeStem('niet-leuk'), 150);
  } else {
